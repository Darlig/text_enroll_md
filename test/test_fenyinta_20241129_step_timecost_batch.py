...
def inference_shift_batch_time(model, test_list, keyword_material, idx2keyword, keyword2idx, shift=False, chunk_size=chunk_size):
    t_sum_sph_emb = 0
    t_sum_cross_att = 0
    t_sum_total = 0
    ...
    for i, obj in enumerate(test_list):
        ...
        while j * chunk_shift + chunk_size <= fbank.size(1):
            ...
            chunk_len = torch.tensor([chunk_size])
            #print_feat(chunk)
    ...
    return result, utt2keyword_idx, t_sum_sph_emb, t_sum_cross_att, t_sum_total
...
if __name__ == '__main__':
    ...
    # print("time cost for positive reference: {:.3f}\n".format(time.time() - t_start))
    ...
    #else:
    t_start = time.time()
    negative_result, _, t_cost_sph_emb, t_cost_cross_att, t_cost_total = inference_shift_batch_time(model, negative_list, (keyword, keyword_len), idx2keyword, keyword2idx, shift=neg_shift)
