...
class TransformerKWSPhone(nn.Module):
        ...
        **kwargs,
        ...
        self.au_transformer_high = nn.ModuleList([
            NM.TransformerLayer(
                ...
                feed_forward=NM.FNNBlock(**au_feed_forward_config),
            ) for _ in range(num_audio_block // 2 )
        ])
    ...
    def forward_unet_self(self, input, mask=None):
        #res = {}
        for i, tf_layer in enumerate(self.au_transformer_low):
            input, att_score = tf_layer(input, mask, cross_input=None)
            #res[i] = input
    ...
    def forward_unet_cross(self, input, mask=None, cross_embedding=None):    
        for i, tf_layer in enumerate(self.au_transformer_high):
            #input = self.skip_trans[i](input + r_res[i])
            input, att_score = tf_layer(input, mask, cross_input=cross_embedding)
        return input
    ...
    def evaluate_sph_emb(self, input_data):
        ...
        sph_emb = self.forward_unet_self(sph_emb, mask=sph_mask)
    ...
    @torch.no_grad()
    def evaluate_cross_attention(self, input_data):
        sph_emb, sph_mask, kw_emb, kw_mask = input_data
        cross_mask = ~NM.combine_mask(sph_mask.squeeze(1), kw_mask.squeeze(1), 1)

        sph_emb = self.forward_unet_cross(sph_emb, mask=sph_mask, cross_embedding=(kw_emb, kw_emb, cross_mask))
