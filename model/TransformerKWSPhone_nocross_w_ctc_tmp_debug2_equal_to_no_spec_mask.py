...
class TransformerKWSPhone_nocross_w_ctc_tmp_debug2_equal_to_no_spec_mask(nn.Module):
    def __init__(
        self,
        audio_net_config,
        kw_net_config,
        ...
        **kwargs,
        ...
        super(TransformerKWSPhone_nocross_w_ctc_tmp_debug2_equal_to_no_spec_mask, self).__init__()
        self.sok = sok
        self.eok = eok
        # detach_config 
        # TODO: looks stupid ....   >_<
    ...
    def forward(self, input_data):
        ...
        target_mask = ~NM.make_mask(target_len).unsqueeze(1)
        ...
        target_flat = target.reshape(-1).to(torch.float32)
        ...
        for i, tf_layer in enumerate(self.au_kw_transformer):
            ...
            # print("target size: {}".format(target.size()))
            det_result_layer_flat = det_result_layer[:,:,0].reshape(-1)
            # det_result_layer_select = det_result_layer_flat[target_mask_flat]
            det_loss_layer = self.det_crit(det_result_layer_flat, target_flat)
