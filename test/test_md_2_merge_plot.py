...
#def plot_multi_roc_curve_for_paper(roc_inputs, test_result_dir, analysis_id, word_py):
#    """
...
def plot_multi_roc_curve_for_paper(roc_inputs, test_result_dir, analysis_id, word_py):
    """
    Plots multiple visually enhanced ROC curves on a single graph.

    Args:
        roc_inputs (list of dict): A list where each dictionary contains:
            'ref_score' (list or np.array): True binary labels.
            'hyp_score' (list or np.array): Target scores.
            'tag' (str): A label for the curve (e.g., 'Model A', 'Model B').
        test_result_dir (str): Directory to save the plot.
        analysis_id (str): A unique ID for the analysis, used in the filename.
        word_py (str): The word or concept the plot represents, used in the title.
    """
    ...
    for curve_data in roc_inputs:
        ref_score = curve_data[0]
        hyp_score = curve_data[1]
        tag = curve_data[2]

        print(len(ref_score), len(hyp_score))
        fpr, tpr, thresholds = roc_curve(ref_score, hyp_score)
        roc_auc = auc(fpr, tpr)
        
        # Plot each curve with its tag and AUC score
        plt.plot(fpr, tpr, lw=2.5, linestyle='-', label=f'{tag} (area = {roc_auc:0.4f})')
        print(f"AUC of ROC for {tag}: {roc_auc:.4f}")
...
def plot_multi_pr_curve_for_paper(pr_inputs, test_result_dir, analysis_id, word_py):
    ...
    #print("=" * 60)
        ...
        plt.plot(recall, precision, lw=2.5, label=f'{tag} PR Curve')
...
def load_scores(result_label_score_path):
    all_results = []
    # y_true = []
    # y_scores = []
    with open(result_label_score_path, 'r') as f:
        ...
        # print(y_scores[:10])
        print("load_scores: {} {}".format(len(y_true), len(y_scores)))
    return y_true, y_scores



def load_gop_scores(result_label_score_path):
    all_results = []
    # y_true = []
    # y_scores = []
    with open(result_label_score_path, 'r') as f:
...
def load_hybrid_gop_scores(data_folder):
    all_y_true = []
    all_y_scores = []
    ...
    y_true = [ 1 - true for true in all_y_true ]
    y_scores = [ 1 - score for score in all_y_scores ]
    print("t_true list: {}".format(y_true[:10]))
    print("t_scores list: {}".format(y_scores[:10]))
    return y_true, y_scores
...
if __name__ == '__main__':
    result_e2e_path1 = "temp_haha_multi_plot/aishell2_cut_gt400_kaldi_lex2_tone_whole_utt_neg_by_lex_pos0.01_change0.8_w0.1_0.1_0.1_0.7_spec_mask_w_psok_hanlp_md_transformer_4sph_4kw_4concat_ctc_det_reverb_perturb_nomix_keyword_init_final_seg_pos_spec0.8_avg35_40_dec_gop/uych_child_31492_unambig/result_label_score.json"
    #result_e2e_path2 = "temp_haha_multi_plot/aishell2_cut_gt400_kaldi_lex2_tone_whole_utt_neg_by_lex_pos0.01_change0.8_w0.1_0.1_0.1_0.7_spec_mask_w_psok_hanlp_md_transformer_4sph_4kw_4concat_ctc_det_reverb_perturb_nomix_keyword_init_final_seg_pos_spec0.8_avg30_38_dec_gop/uych_child_31492_unambig/result_label_score.json"
    result_ctc_gop_path = "temp_haha_multi_plot/aishell2_cut_gt400_kaldi_lex2_tone_whole_utt_neg_by_lex_pos0.01_change0.8_w0.1_0.1_0.1_0.7_spec_mask_w_psok_hanlp_md_transformer_4sph_4kw_4concat_ctc_det_reverb_perturb_nomix_keyword_init_final_seg_pos_spec0.8_avg35_40_dec_gop/uych_child_31492_unambig/result_gop.txt"
    #result_ctc_gop_path = "temp_haha_multi_plot/aishell2_cut_gt400_kaldi_lex2_tone_whole_utt_neg_by_lex_pos0.01_change0.8_w0.1_0.1_0.1_0.7_spec_mask_w_psok_hanlp_md_transformer_4sph_4sph_ctc_reverb_perturb_nomix_keyword_init_final_seg_pos_spec0.8_avg30_38_gop.dragon04/uych_child_31492_unambig/result_gop.txt"
    #result_e2e_path = "result/aishell2_cut_gt400_kaldi_lex2_tone_whole_utt_neg_by_lex_pos0.01_change0.8_w0.1_0.1_0.1_0.7_spec_mask_w_psok_hanlp_md_transformer_4sph_4kw_4concat_ctc_det_reverb_perturb_nomix_keyword_init_final_seg_pos_spec0.8_avg35_40_dec_gop/uych_child_31492_unambig/result_label_score.json"
    #result_ctc_gop_path = "result/aishell2_cut_gt400_kaldi_lex2_tone_whole_utt_neg_by_lex_pos0.01_change0.8_w0.1_0.1_0.1_0.7_spec_mask_w_psok_hanlp_md_transformer_4sph_4sph_ctc_reverb_perturb_nomix_keyword_init_final_seg_pos_spec0.8_avg35_40_gop.dragon04_2/uych_child_31492_unambig/result_label_score.json"
    result_hybrid_gop_dir = "temp_haha_multi_plot/ph_gop_test_uych_child_31492_unambig/"
    out_dir = "result/paper_plot"
    y_true_e2e1, y_scores_e2e1 = load_scores(result_e2e_path1)
    #y_true_e2e2, y_scores_e2e2 = load_scores(result_e2e_path2)
    #y_true_e2e, y_scores_e2e = load_scores(result_e2e_path)
    y_true_ctc_gop, y_scores_ctc_gop = load_gop_scores(result_ctc_gop_path)
    y_true_hybrid_gop, y_scores_hybrid_gop = load_hybrid_gop_scores(result_hybrid_gop_dir)

    
    roc_inputs = []
    roc_inputs.append([y_true_e2e1, y_scores_e2e1, "Ours_E2E"])
    #roc_inputs.append([y_true_e2e2, y_scores_e2e2, "Ours_E2E2"])
