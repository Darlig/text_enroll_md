...
class Trainer():
    ...
    def init_opt_model(self):
        ...
        if start_epoch != 0:
            ...
            self.load_ckpt_kw_module(self.data_config['pretrained_kw_ckpt'])
            self.fix_kw_module()
            self.global_step = 0
            if self.rank == 0:
    ...
    def load_ckpt_kw_module(self, ckpt):
        ckpt_dict = torch.load(ckpt, map_location='cpu')
        model = ckpt_dict['model']

    # ['au_conv', 'au_conv_trans', 'au_kw_transformer', 'au_trans', 'au_transformer', 'det_net', 'kw_trans', 'kw_transformer', 'phn_asr_crit', 'phn_emb']
        kw_trans_state_dict = {re.sub("^kw_trans\.", "", k) : v for k, v in model.items() if k.startswith('kw_trans.')}
        print(kw_trans_state_dict.keys())
        # print(kw_trans_state_dict['w1.weight'])
        self.model.kw_trans.load_state_dict(kw_trans_state_dict)
        kw_transformer_state_dict = {re.sub("^kw_transformer\.", "", k) : v for k, v in model.items() if k.startswith('kw_transformer.')}
        print(kw_transformer_state_dict.keys())
        print(kw_transformer_state_dict['3.self_att.q.bias'])
        self.model.kw_transformer.load_state_dict(kw_transformer_state_dict)
    
    def fix_kw_module(self):
        for param in self.model.kw_trans.parameters():
            param.requires_grad = False
        for param in self.model.kw_transformer.parameters():
            param.requires_grad = False
    ...
    def record_step(self, r_loss):
        ...
        # print(self.model.kw_transformer[3].self_att.q.bias)
