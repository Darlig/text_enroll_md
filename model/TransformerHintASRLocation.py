...
class TransformerHintASRLocation(nn.Module):
    def __init__(
        self,
        audio_net_config,
        vocab_net_config,
        ...
        super(TransformerHintASRLocation, self).__init__()
    ...
    def forward(self, input_data):
        ...
        kw_mask = kw_mask.squeeze(1)
        kw_mask = torch.bernoulli(kw_mask * 0.5).unsqueeze(-1).to(sph_emb.device)
        phn_emb = phn_emb * kw_mask
        phn_emb = self.vocab_pos_emb(phn_emb)
        ...
        # compute hint attent speech
        # cross_context, cross_att = self.location_cross_att(
        #    phn_emb, sph_emb, sph_emb, mask=cross_mask.transpose(-2,-1)
        # )
        # sementic = cross_context[:,0,:]
        # sementic = sementic.unsqueeze(1).repeat(1,sph_emb.size(1),1)

        # concat sementic feats with speech feats
        # sph_emb = torch.cat([sph_emb, sementic], dim=-1)
        # phn_emb = cross_context[:,1:,:]

        # compute asr result  & ctc loss
        # asr_loss, asr_hyp = self.asr_crit(
        #     sph_emb, phn_label, sph_len, phn_len, return_hyp=True
        # )
        cross_context = self.hint_net(cross_context[:,1:,:])
        num_class = cross_context.size(-1)
        cross_context = cross_context.view(-1, num_class)

        # remove semantic token
        kw_label = kw_label[:,1:].reshape(-1)
        kw_mask2 = kw_mask2.squeeze(1)
        kw_mask2 = kw_mask2[:,1:].reshape(-1)
        rec_loss = self.xent(cross_context[kw_mask2], kw_label[kw_mask2])
        asr_loss = self.l1 * asr_loss # average batch
        rec_loss = self.l2 * rec_loss * 0.3

        ## compute align loss
        total_loss = asr_loss + rec_loss
        detail_loss = {
            'asr_loss': asr_loss.clone().detach(),
            'rec_loss': rec_loss.clone().detach()
