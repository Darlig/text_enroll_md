...
NONE_TENSOR_KEY = [
    ...
    'corrupt', 'self_corruption', 'none_target_corruption', 'nframes'
]
CTC_KEY = ['label', 'crpt_label', 'phn_label', 'bpe_label', 'c_phn_label', 'c_bpe_label'] # to be extended
...
def inject_special_token(
        ...
        positive=True, keyword_pos=None, special_token={}, n=None, bpe_label=None, bpe_candidate=None
    ):
    ...
    if (not positive) and (TEXT_SPEC_TOKEN['punk'] != None):
        ...
        #new_label = [TEXT_SPEC_TOKEN['unk'], TEXT_SPEC_TOKEN['sok']] + [TEXT_SPEC_TOKEN['unk'] for x in range(n//5)] + [TEXT_SPEC_TOKEN['eok'], TEXT_SPEC_TOKEN['unk']]
        #new_label = torch.tensor(new_label)
        #new_label = torch.tensor([
        #    TEXT_SPEC_TOKEN['unk'], TEXT_SPEC_TOKEN['sok'], TEXT_SPEC_TOKEN['unk'], TEXT_SPEC_TOKEN['eok'], TEXT_SPEC_TOKEN['unk']
        #])
        #new_label = torch.tensor([TEXT_SPEC_TOKEN['sok'] for x in range(n//100)])
    ...
    #if (positive) and (TEXT_SPEC_TOKEN['unk'] != None):
    #    new_label = [TEXT_SPEC_TOKEN['unk']] + new_label + [TEXT_SPEC_TOKEN['unk']]
...
def process_speech_feats(data, config, egs_format=False):
    for sample in data:
        ...
        #zero_padding = torch.zeros(32000)
        #for i, tmp in enumerate(feats):
        #    if i == 0:
        #        save_tmp = tmp.view(-1)
        #    else:
        #        save_tmp = torch.cat([save_tmp, zero_padding, tmp.view(-1)], dim=0)
        #torchaudio.save(f"{sample['key']}.wav", save_tmp.view(1,-1), bits_per_sample=16, encoding="PCM_S", sample_rate=16000)
        #save_wav(feats[0], sample['key'])
...
def process_sampled_keyword_from_label(
    ...
    for sample in data:
        ...
        kw, new_phn_label, new_bpe_label, kw_pos = inject_special_token(
            ...
            keyword_pos=kw_pos, special_token=special_token, n=nframes, bpe_label=new_bpe_label, bpe_candidate=bpe_candidate
        )
        #with open(f"{sample['key']}.txt", 'w') as wf:
        #    wf.write(str(kw)+"\n")
        #    wf.write(str(new_phn_label)+"\n")
        #    wf.write(str(new_bpe_label)+"\n")
