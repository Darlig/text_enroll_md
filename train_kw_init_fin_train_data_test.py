...
class Trainer():
    ...
    def strip_special_token(self, keyword, target_level):
        ...
        if 'phone' in target_level:
            keyword = keyword[1::2]
        # print("keyword after strip special token: ", keyword)
        # exit()
        return keyword
        
    def result_extract(self, det_result, target_level):
        if 'word' in target_level:
    ...
    def train(self):
        ...
        target_level = self.data_config['keyword_config']['config']['target_level']
        result_score_path = self.exp_config['result_score_path']
        if not os.path.exists(os.path.dirname(result_score_path)):
            os.makedirs(os.path.dirname(result_score_path))
        f_score = open(result_score_path, 'w')
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            for batch_id, data in enumerate(self.tr_loader):
                ...
                for i in range(len(det_result)):
                    f_score.write("{}.{}\t{}\t{}\t{}\n".format('train_data', i, target[0][i], det_result[i], keyword[i]))
                # 暂时不处理返回值，跳过优化步骤
