...
def process_speech_feats(data: Iterator[Dict], config: Dict[Any, Any]) -> Iterator[Dict]:
    for sample in data:
        ...
        speech_feats = [torch.load(x) for x in speech_feats]
        #speech_feats = [INPUT_DATA_LOADER[input_data_type](x) for x in speech_feats]
        sample_rate = INPUT_DATA_LOADER['read_sr'](speech_feats[0]) if input_data_type == 'raw' else speech_feats  # TODO: this is a temp code 
